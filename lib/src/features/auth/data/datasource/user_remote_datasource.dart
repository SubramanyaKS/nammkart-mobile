...
Future<String> userSignup(user) async{
    final response = await http.post(Uri.parse('${Environment.backendUrl}/api/users/auth/register',),headers: {
...
Future<String> userLogin(user) async{
    final response = await http.post(Uri.parse('${Environment.backendUrl}/api/users/auth/login',),headers: {
      "Content-Type": "application/json",
    },body: jsonEncode(user));
    if(response.statusCode==201 || response.statusCode==200){
      return response.body.toString();
...
Future<UserEntity> userByEmail(token) async{
  try{
    final response = await http.get(Uri.parse('${Environment.backendUrl}/api/users/auth/user/email'),headers: {
